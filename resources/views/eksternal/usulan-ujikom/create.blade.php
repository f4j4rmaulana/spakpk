        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">Buat Usulan Uji Kompetensi</h5>
                </div>
                <div class="card-body">
                        <form action="{{ route('eksternal.usulan-ujikom.store') }}" method="POST">
                            @csrf
                                <div class="mb-3">
                                ...
                                <div class="mb-3 d-flex justify-content-end gap-1">
                                    <a href="{{ route('eksternal.usulan-ujikom.index') }}" class="btn btn-link shadow-none" role="button">Batal</a>
...
<script>
    $(document).ready(function(){
        $('#jenis_ujikom_id').select2({
            ...
            ajax:{
                url:route('eksternal.usulan-ujikom.ajaxGetJenisUjikom'),
                dataType:'json',
                processResults:data=>{
                    return {
...
<script>
    $(document).ready(function(){
        $('#ujikom_id').select2({
            ...
            // placeholder:'Pilih Pengusul',
            ajax:{
                url:route('eksternal.usulan-ujikom.ajaxGetUjikom'),
                dataType:'json',
                processResults:data=>{
                    return {
                        results:data.map(res=>{
