                ...
                <div class="card-body">
                    <form action="{{ route('admin.jenis-pelatihan.update', $jenisPelatihan->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control form-control-lg {{ hasError($errors, 'nama') }}" id="nama" name="nama" value="{{old('nama', $jenisPelatihan->nama)}}" required autofocus>
                            <x-input-error :messages="$errors->get('nama')" class="mt-1" />
                        </div>
                        <div class="mb-3">
                            <label for="deskripsi" class="form-label">Deskripsi</label>
                            <textarea class="form-control form-control-lg {{ hasError($errors, 'deskripsi') }}" id="deskripsi" name="deskripsi" rows="2" required autofocus>{{old('deskripsi', $jenisPelatihan->deskripsi)}}</textarea>
