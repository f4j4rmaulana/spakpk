        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">Edit Usulan Pelatihan</h5>
                </div>
                <div class="card-body">
                        <form action="{{ route('internal.usulan-pelatihan.update', Crypt::encryptstring($usulanPelatihan->id)) }}" method="POST">
                            @csrf
                            @method('PUT')
                                <div class="mb-3">
                                ...
                                <div class="mb-3 d-flex justify-content-end gap-1">
                                    <a href="{{ route('internal.usulan-pelatihan.index') }}" class="btn btn-link shadow-none" role="button">Batal</a>
...
<script>
    $(document).ready(function(){
        $('#jenis_pelatihan_id').select2({
            ...
            ajax:{
                url:route('internal.usulan-pelatihan.ajaxGetJenisPelatihan'),
                dataType:'json',
                processResults:data=>{
                    return {
...
<script>
    $(document).ready(function(){
        $('#pelatihan_id').select2({
            ...
            // placeholder:'Pilih Pengusul',
            ajax:{
                url:route('internal.usulan-pelatihan.ajaxGetPelatihan'),
                dataType:'json',
                processResults:data=>{
                    return {
                        results:data.map(res=>{
